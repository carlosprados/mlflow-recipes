...
from opengate.recipes.cards import histogram_generator
    ...
    for key in df:
        ...
        if feat.type in (fs_proto.INT, fs_proto.FLOAT):
            ...
            if current_column_value.dtype == bool:
                current_column_value = current_column_value.replace(
            ...
            feat_stats.avg_length = (
                ...
                if not is_current_column_boolean_type
                and not current_column_value.isnull().all()
                else 0
            )
            ...
            for val_index, val in enumerate(sorted_vals):
                try:
                    if sys.version_info.major < 3 or isinstance(
                        val[1], (bytes, bytearray)
                    ):
