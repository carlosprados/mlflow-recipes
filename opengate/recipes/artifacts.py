        ...
        if run_id:
            with _use_tracking_uri(self._tracking_uri), chdir(self._recipe_root):
                return mlflow.pyfunc.load_model(
                    f"runs:/{run_id}/{self._step_name}/model"
                )
        log_artifact_not_found_warning(self._name, self._step_name)
        return None
        ...
        if run_id:
            with _use_tracking_uri(self._tracking_uri), chdir(self._recipe_root):
                return mlflow.sklearn.load_model(
                    f"runs:/{run_id}/{self._step_name}/transformer"
                )
        log_artifact_not_found_warning(self._name, self._step_name)
        return None
...
class ModelVersionArtifact(Artifact):
    def __init__(self, name, recipe_root, step_name, tracking_uri):
        self._name = name
        self._path = get_step_output_path(
            recipe_root, step_name, "registered_model_version.json"
        )
        self._recipe_root = recipe_root
        self._step_name = step_name
        self._tracking_uri = tracking_uri
...
class HyperParametersArtifact(Artifact):
    def __init__(self, name, recipe_root, step_name):
        self._name = name
        self._path = get_step_output_path(
            recipe_root, step_name, "best_parameters.yaml"
        )
...
class RegisteredModelVersionInfo:
    ...
    def from_json(cls, path):
        ...
        return cls(
            name=registered_model_info_dict[
                RegisteredModelVersionInfo._KEY_REGISTERED_MODEL_NAME
            ],
