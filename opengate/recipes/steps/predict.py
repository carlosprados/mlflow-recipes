...
class PredictStep(BaseStep):
    ...
    def _build_profiles_and_card(self, scored_sdf) -> BaseCard:
        ...
        if not self.skip_data_profiling:
            ...
            if scored_size > _MAX_PROFILE_SIZE:
                _logger.info(
                    "Sampling scored dataset for profiling because dataset size is large."
                )
    ...
    def _run(self, output_directory):
        ...
        if self.save_mode in ["default", "error", "errorifexists"]:
            if output_format == "parquet" or output_format == "delta":
                ...
                try:
                    output_populated = spark._jsparkSession.catalog().tableExists(
                        output_location
                    )
                except Exception:
        ...
        if output_format in ["parquet", "delta"]:
            scored_sdf.coalesce(1).write.format(output_format).mode(
                self.save_mode
            ).save(output_location)
        else:
            scored_sdf.write.format("delta").mode(self.save_mode).saveAsTable(
                output_location
            )
    ...
    def from_recipe_config(cls, recipe_config, recipe_root):
        ...
        if (
            recipe_config.get("steps", {}).get("predict", {}).get("output", {})
            is not None
        ):
            step_config.update(
                recipe_config.get("steps", {}).get("predict", {}).get("output", {})
