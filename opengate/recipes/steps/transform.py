...
from opengate.recipes.utils.tracking import TrackingConfig, get_recipe_tracking_config
...
def _validate_user_code_output(transformer_fn):
    ...
    if transformer is not None and not (
        hasattr(transformer, "fit") and callable(transformer.fit)
    ):
...
class TransformStep(BaseStep):
    ...
    def _run(self, output_directory):
        ...
        if (
            "transformer_method" not in self.step_config
        ...
        if method_config and self.step_config["using"] == "custom":
            transformer_fn = getattr(
                importlib.import_module(_USER_DEFINED_TRANSFORM_STEP_MODULE),
                method_config,
            )
        ...
        transformer.fit(
            train_df.drop(columns=[self.target_col]), train_df[self.target_col]
        )
        ...
        def transform_dataset(dataset):
            ...
            if not isinstance(transformed_features, pd.DataFrame):
                ...
                columns = _get_output_feature_names(
                    transformer, num_features, features.columns
                )
    ...
    def _build_profiles_and_card(
        self, train_df, train_transformed, transformer
    ) -> BaseCard:
        # Build card
        ...
        if not self.skip_data_profiling:
            ...
            card.add_tab(
                "Data Profile (Train Transformed)", "{{PROFILE}}"
            ).add_pandas_profile("PROFILE", train_transformed_profile)
        ...
        card.add_tab("Transformer", "{{TRANSFORMER}}").add_html(
            "TRANSFORMER", transformer_repr
        )
        ...
        card.add_tab("Input Schema", "{{INPUT_SCHEMA}}").add_html(
            ...
            BaseCard.render_table(
                {"Name": n, "Type": t} for n, t in train_df.dtypes.items()
            ),
        )
