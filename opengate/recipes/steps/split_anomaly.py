...
def _make_elem_hashable(elem):
    if isinstance(elem, list):
        ...
        return tuple(
            (_make_elem_hashable(k), _make_elem_hashable(v)) for k, v in elem.items()
        )
...
def _run_split(task, input_df, split_ratios, target_col):
    ...
    if task == "anomaly":
        if target_col is not None:
            # Labeled anomaly detection → Use stratified splitting
            return _perform_stratified_split_per_class(input_df, split_ratios, target_col)
        else:
            # Unsupervised anomaly detection → Use normal split
...
def _perform_stratified_split_per_class(input_df, split_ratios, target_col):
    ...
    with ThreadPool(os.cpu_count() or _NUM_DEFAULT_CPUS) as p:
        ...
        train_df, validation_df, test_df = [
            pd.concat(x) for x in list(zip(*zipped_dfs))
        ]
...
def _perform_split(input_df, split_ratios, n_jobs=-1):
    ...
    train_df, validation_df, test_df = _get_split_df(
        input_df, hash_buckets, split_ratios
    )
    return train_df, validation_df, test_df
...
def _get_split_df(input_df, hash_buckets, split_ratios):
    ...
    if len(empty_splits) > 0:
        _logger.warning(
            f"The following input dataset splits are empty: {','.join(empty_splits)}."
        )
    return train_df, validation_df, test_df
...
class SplitAnomalyStep(BaseStep):
    def _validate_and_apply_step_config(self):
        ...
        if self.target_col is None and self.task != _ANOMALY:
            raise MlflowException(
        ...
        if self.step_config["using"] == "split_ratios":
            self.split_ratios = self.step_config.get(
            ...
            if not (
                ...
                and all(
                    isinstance(x, (int, float)) and x > 0 for x in self.split_ratios
                )
            ):
                raise MlflowException(
        ...
        if (
            "split_method" not in self.step_config
    ...
    def _build_profiles_and_card(self, train_df, validation_df, test_df) -> BaseCard:
            ...
            if self.task == "classification":
                if self.positive_class is not None:
                    ...
                    class_weights = sorted(
                        class_weights, key=lambda x: x[1], reverse=True
                    )
        ...
        (
            ...
            .add_markdown(
                "NUM_DROPPED_ROWS",
                f"**Number of dropped rows:** `{self.num_dropped_rows}`",
            )
            .add_markdown(
                "TRAIN_SPLIT_NUM_ROWS",
                f"**Number of train dataset rows:** `{len(train_df)}`",
            )
            .add_markdown(
                "VALIDATION_SPLIT_NUM_ROWS",
            ...
            .add_markdown(
                "TEST_SPLIT_NUM_ROWS",
                f"**Number of test dataset rows:** `{len(test_df)}`",
            )
        )
    ...
    def _validate_and_execute_custom_split(self, split_fn, input_df):
        ...
        train_df = input_df[copy_df["split"] == SplitValues.TRAINING.value].reset_index(
            drop=True
        )
        validation_df = input_df[
            copy_df["split"] == SplitValues.VALIDATION.value
        ].reset_index(drop=True)
        test_df = input_df[copy_df["split"] == SplitValues.TEST.value].reset_index(
            drop=True
        )
    ...
    def _run(self, output_directory):
        ...
        validate_classification_config(
            self.task, self.positive_class, input_df, self.target_col
        )
        ...
        if self.target_col not in input_df.columns and self.task != _ANOMALY:
            raise MlflowException(
                f"Target column '{self.target_col}' not found in ingested dataset.",
                error_code=INVALID_PARAMETER_VALUE,
            )

        # Make optional target_col for anomaly
        drop_subset = [self.target_col] if self.target_col else None
        input_df = input_df.dropna(how="any", subset=drop_subset)
        ...
        post_split_filter_config = self.step_config.get(
            "post_split_filter_method", None
        )
        if post_split_config is not None:
            ...
            _logger.debug(
                f"Running {post_split_config} on train, validation and test datasets."
            )
            (
                train_df,
                validation_df,
    ...
    def get_artifacts(self):
        ...
        return [
            ...
            DataframeArtifact(
                "test_data", self.recipe_root, self.name, _OUTPUT_TEST_FILE_NAME
            ),
        ]
