...
class EvaluateIsolationForestStep(BaseStep):
    ...
    def _validate_validation_criteria(self):
        ...
        """
        val_metrics = {
            vc["metric"] for vc in self.step_config.get("validation_criteria", [])
        }
        if not val_metrics:
            return
                ...
                for dataset_name, dataset, evaluator_config in (
                    ...
                    (
                        ...
                        test_df,
                        {
                            ...
                            "metric_prefix": DatasetSplit.VALIDATION.value,
                        },
                    ),
                ):
    ...
    def _validate_model(self, eval_metrics, output_directory):
        ...
        if validation_criteria:
            ...
            self.model_validation_status = (
                "VALIDATED"
                if all(cr.validated for cr in validation_results)
                else "REJECTED"
            )
        else:
            self.model_validation_status = "UNKNOWN"
    ...
    def _build_profiles_and_card(
        ...
        if validation_results is not None:
            ...
            card.add_tab(
                "Model Validation", "{{ METRIC_VALIDATION_RESULTS }}"
            ).add_html(
        ...
        if self.recipe == "classification/v1":
            ...
            if os.path.exists(lift_curve_path):
                ...
                classifiers_plot_tab.add_image(
                    "LIFT_CURVE_PLOT", lift_curve_path, width=400
                )
            ...
            if os.path.exists(pr_curve_path):
                ...
                classifiers_plot_tab.add_image(
                    "PR_CURVE_PLOT", pr_curve_path, width=400
                )
            ...
            if os.path.exists(roc_curve_path):
                ...
                classifiers_plot_tab.add_image(
                    "ROC_CURVE_PLOT", roc_curve_path, width=400
                )
        ...
        def _add_shap_plots(card):
            ...
            shap_plot_tab.add_image(
                "SHAP_BEESWARM_PLOT", shap_beeswarm_plot_path, width=800
            )
        ...
        run_summary_card_tab = card.add_tab(
            ...
            "VALIDATION_STATUS",
            f"**Validation status:** `{self.model_validation_status}`",
        )
        ...
        if run_url is not None:
            ...
            run_summary_card_tab.add_markdown(
        ...
        if model_url is not None:
            run_summary_card_tab.add_html(
                "MODEL_URI",
                f"<b>MLflow Model URI:</b> <a href={model_url}>{model_uri}</a>",
            )
        else:
            run_summary_card_tab.add_markdown(
                "MODEL_URI", f"**MLflow Model URI:** `{model_uri}`"
            )
