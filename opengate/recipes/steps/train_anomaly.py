...
from opengate.recipes.utils.anomaly_evaluate_util import EvaluateAnomalyModel, process_predictions, \
    preprocess_anomaly_data, calculate_threshold_quantile
...
class TrainAnomalyStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        ...
        if "tuning" in self.step_config:
            if "enabled" in self.step_config["tuning"] and isinstance(
                ...
                self.step_config["tuning_enabled"] = self.step_config["tuning"][
                    "enabled"
                ]
            else:
                raise MlflowException(
            ...
            if self.step_config["tuning_enabled"]:
                if "sample_fraction" in self.step_config["tuning"]:
                    sample_fraction = float(
                        self.step_config["tuning"]["sample_fraction"]
                    )
        ...
        self.rebalance_training_data = self.step_config.get(
        ...
        self.user_defined_custom_metrics = {
            metric.name: metric for metric in custom_metrics
        }
        ...
        if (
            ...
            and self.primary_metric not in self.evaluation_metrics
        ):
            raise MlflowException(
    ...
    def construct_search_space_from_yaml(cls, params):
        ...
        for param_name, param_details in params.items():
            if "values" in param_details:
                ...
                search_space[param_name] = hp_tuning_fn(
                    param_name, **param_details_to_pass
                )
            else:
                raise MlflowException(
    ...
    def _run(self, output_directory):
        ...
        try:
            ...
            X_train, y_train = (
                ...
                train_df[self.target_col] if self.target_col is not None else train_df,
            )
            ...
            raw_X_train = raw_train_df.drop(columns=columns_to_drop)
            ...
            if isinstance(train_predictions, pd.DataFrame) and {
                ...
                predicted_training_data = raw_train_df.assign(
                    ...
                    predicted_score=train_predictions[
                        f"{self.predict_prefix}score"
                    ].values,
                )
                if self.positive_class:
            ...
            if self.step_config["tuning_enabled"]:
                try:
                    tuning_df = self._get_tuning_df(
                        run, params=best_estimator_params.keys()
                    )
                except Exception as e:
                    _logger.warning(
                        "Failed to build tuning results table due to unexpected failure: %s",
                        e,
                    )
    ...
    def _get_user_defined_estimator(
        self, X_train, y_train, validation_df, run, output_directory
    ):
        ...
        if self.step_config["tuning_enabled"]:
            estimator_hardcoded_params, best_hp_params = (
                self._tune_and_get_best_estimator_params(
                    run.info.run_id,
            ...
            default_params_keys = (
                all_estimator_params.keys() - best_combined_params.keys()
            )
            ...
            self._write_best_parameters_outputs(
                output_directory, default_params=default_params
            )
        return estimator
    ...
    def _resolve_estimator_plugin(self, plugin_str, X_train, y_train, output_directory):
        ...
        estimator_fn = importlib.import_module(
            f"opengate.recipes.steps.{plugin_str}"
        ...
        self._write_best_parameters_outputs(
            output_directory, automl_params=best_parameters
        )
        return estimator
    ...
    def _resolve_estimator(
        ...
        if using_plugin == "custom":
            ...
            return self._resolve_estimator_plugin(
                using_plugin, X_train, y_train, output_directory
            )
    ...
    def _get_leaderboard_df(self, run, eval_metrics):
                ...
                leaderboard_items.append(
                    {
                        ...
                        **{
                            ...
                            for metric_name, metric_key in zip(
        ...
        top_leaderboard_item_index_values = ["Best", "2nd Best"][
            : len(top_leaderboard_items)
        ]
        ...
        for i, leaderboard_item in enumerate(leaderboard_items):
            ...
            if (
                primary_metric_greater_is_better and latest_value >= historical_value
            ) or (
                not primary_metric_greater_is_better
                and latest_value <= historical_value
            ):
    ...
    def _get_tuning_df(self, run, params=None):
        ...
        if params:
            ...
            tuning_runs = tuning_runs.filter(
                [f"metrics.{self.primary_metric}", *params]
            )
        else:
    ...
    def _build_step_card(
        ...
        def row_style(row):
            if (
                row.Metric == self.primary_metric
                or row.Metric in self.user_defined_custom_metrics
            ):
        ...
        if not self.skip_data_profiling:
            ...
            card.add_tab(
                "Data Profile (Predictions)", "{{PROFILE}}"
            ).add_pandas_profile("PROFILE", pred_and_error_df_profile)
        ...
        card.add_tab("Model Architecture", "{{MODEL_ARCH}}").add_html(
            "MODEL_ARCH", model_repr
        )
        ...
        card.add_tab("Model Schema", "{{MODEL_SCHEMA}}").add_html(
            "MODEL_SCHEMA",
            '<div style="display: flex">{tables}</div>'.format(
                tables="\n".join(schema_tables)
            ),
        )
        ...
        if not worst_examples_df.empty:
            (
                card.add_tab(
                    "Worst Predictions", "{{ WORST_EXAMPLES_TABLE }}"
                ).add_html(
        ...
        if calibrated_plot:
            calibrated_plot_location = os.path.join(
                output_directory, "calibrated_plot_location"
            )
        ...
        if leaderboard_df is not None:
            (
                card.add_tab("Leaderboard", "{{ LEADERBOARD_TABLE }}").add_html(
                    "LEADERBOARD_TABLE",
                    BaseCard.render_table(leaderboard_df, hide_index=False),
                )
            )
        ...
        if tuning_df is not None:
            ...
            tuning_trials_card_tab.add_html(
                "TUNING_TABLE_TITLE", "<b>Tuning results:</b><br>"
            )
            tuning_trials_card_tab.add_html(
                "TUNING_TABLE",
        ...
        if run_url is not None:
            run_card_tab.add_html(
                "RUN_ID",
                f"<b>MLflow Run ID:</b> <a href={run_url}>{run_id}</a><br><br>",
            )
        else:
        ...
        if model_url is not None:
            run_card_tab.add_html(
                "MODEL_URI",
                f"<b>MLflow Model URI:</b> <a href={model_url}>{model_uri_path}</a>",
            )
        else:
            run_card_tab.add_markdown(
                "MODEL_URI", f"**MLflow Model URI:** `{model_uri_path}`"
            )
    ...
    def environment(self):
        environ = get_databricks_env_vars(
    ...
    def get_artifacts(self):
        ...
        return [
            ...
            RunArtifact(
                "run", self.recipe_root, self.name, self.tracking_config.tracking_uri
            ),
    ...
    def _tune_and_get_best_estimator_params(
        ...
        def objective(
            X_train, y_train, validation_df, hyperparameter_args, on_worker=False
        ):
            run_name = self.tracking_config.run_name
            if on_worker:
                client = MlflowClient()
            ...
            with mlflow.start_run(**run_args) as tuning_run:
                ...
                autologged_params = mlflow.get_run(
                    run_id=tuning_run.info.run_id
                ).data.params
                manual_log_params = {}
        ...
        if "early_stop_fn" in tuning_params:
            try:
                train_module_name, early_stop_fn_name = tuning_params[
                    "early_stop_fn"
                ].rsplit(".", 1)
            except ValueError:
                early_stop_fn_name = tuning_params["early_stop_fn"]
                train_module_name = _USER_DEFINED_TRAIN_STEP_MODULE
            early_stop_fn = getattr(
                importlib.import_module(train_module_name), early_stop_fn_name
            )
    ...
    def _log_estimator_to_mlflow(self, estimator, X_train_sampled, on_worker=False):
        ...
        if hasattr(estimator, "best_score_") and (
            type(estimator.best_score_) in [int, float]
        ):
    ...
    def _write_best_parameters_outputs(
        ...
        if best_hp_params or best_hardcoded_params or automl_params or default_params:
            best_parameters_path = os.path.join(
                output_directory, "best_parameters.yaml"
            )
            ...
            with open(best_parameters_path, "a") as file:
                self._write_one_param_output(
                    automl_params or {}, file, "automl parameters"
                )
                self._write_one_param_output(
                    best_hp_params or {}, file, "tuned hyperparameters"
                )
                self._write_one_param_output(
                    best_hardcoded_params or {}, file, "hardcoded parameters"
                )
                self._write_one_param_output(
                    default_params or {}, file, "default parameters"
                )
