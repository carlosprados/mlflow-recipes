...
from opengate.recipes.utils.metrics import RecipeMetric, _load_custom_metrics
...
def _create_sklearn_metric_flaml(
    metric_name: str, coeff: int, avg: str = "binary"
) -> callable:
    def sklearn_metric(
        X_val,
        y_val,
...
def _create_model_automl(
    ...
    try:
        if (
            primary_metric in _MLFLOW_TO_FLAML_METRICS
            and primary_metric in evaluation_metrics
        ):
            metric = _MLFLOW_TO_FLAML_METRICS[primary_metric]
            if (
                primary_metric == "roc_auc"
                and extended_task == "classification/multiclass"
            ):
                metric = "roc_auc_ovr"
        elif (
            primary_metric in _SKLEARN_METRICS and primary_metric in evaluation_metrics
        ):
            ...
            metric = _create_custom_metric_flaml(
                ...
                primary_metric,
                -1 if evaluation_metrics[primary_metric].greater_is_better else 1,
                _load_custom_metrics(recipe_root, [evaluation_metrics[primary_metric]])[
                    0
                ],
            )
        else:
