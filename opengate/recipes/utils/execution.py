...
def run_recipe_step(
    ...
    for step in recipe_steps[1 : target_step_index + 1]:
        ...
        if (
            step_state.last_updated_timestamp
...
def get_step_output_path(
    recipe_root_path: str, step_name: str, relative_path: str
) -> str:
    """
...
def _write_updated_step_confs(
    recipe_steps: list[BaseStep], execution_directory_path: str
) -> None:
    """
...
def get_or_create_base_execution_directory(recipe_root_path: str) -> str:
    ...
    execution_dir_path = os.path.abspath(
        ...
        or os.path.join(
            os.path.expanduser("~"), ".mlflow", "recipes", execution_directory_basename
        )
    )
...
def _get_step_output_directory_path(
    execution_directory_path: str, step_name: str
) -> str:
    """
...
class _ExecutionPlan:
    ...
    def __init__(
        self, rule_name, output_lines_of_make: list[str], recipe_step_names: list[str]
    ):
        steps_to_run = self._parse_output_lines(output_lines_of_make)
        self.steps_cached = self._infer_cached_steps(
            rule_name, steps_to_run, recipe_step_names
        )
...
class _MakefilePathFormat:
    ...
    def _get_formatted_path(
        ...
        recipe_root_posix_path = pathlib.PurePosixPath(
            pathlib.Path(replacement_path).as_posix()
        )
    ...
    def __format__(self, path_spec: str) -> str:
        ...
        if path_spec.startswith(
            _MakefilePathFormat._RECIPE_ROOT_PATH_PREFIX_PLACEHOLDER
        ):
            ...
            _MakefilePathFormat._EXECUTION_DIRECTORY_PATH_PREFIX_PLACEHOLDER
        ):
            return self._get_formatted_path(
                path_spec=path_spec,
...
ingest:
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.ingest import IngestStep; IngestStep.from_step_config_path(step_config_path='{path:exe/steps/ingest/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/ingest/outputs}')"
...
steps/%/outputs/train.parquet steps/%/outputs/validation.parquet steps/%/outputs/test.parquet: {path:prp/steps/split.py} steps/ingest/outputs/dataset.parquet steps/split/conf.yaml
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.split import SplitStep; SplitStep.from_step_config_path(step_config_path='{path:exe/steps/split/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/split/outputs}')"
...
steps/%/outputs/transformer.pkl steps/%/outputs/transformed_training_data.parquet steps/%/outputs/transformed_validation_data.parquet: {path:prp/steps/transform.py} steps/split/outputs/train.parquet steps/split/outputs/validation.parquet steps/transform/conf.yaml
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.transform import TransformStep; TransformStep.from_step_config_path(step_config_path='{path:exe/steps/transform/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/transform/outputs}')"
...
steps/%/outputs/model steps/%/outputs/run_id: {path:prp/steps/train.py} {path:prp/steps/custom_metrics.py} steps/transform/outputs/transformed_training_data.parquet steps/transform/outputs/transformed_validation_data.parquet steps/split/outputs/train.parquet steps/split/outputs/validation.parquet steps/transform/outputs/transformer.pkl steps/train/conf.yaml
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.train import TrainStep; TrainStep.from_step_config_path(step_config_path='{path:exe/steps/train/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/train/outputs}')"
...
steps/%/outputs/model_validation_status: {path:prp/steps/custom_metrics.py} steps/train/outputs/model steps/split/outputs/validation.parquet steps/split/outputs/test.parquet steps/train/outputs/run_id steps/evaluate/conf.yaml
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.evaluate import EvaluateStep; EvaluateStep.from_step_config_path(step_config_path='{path:exe/steps/evaluate/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/evaluate/outputs}')"
...
steps/%/outputs/registered_model_version.json: steps/train/outputs/run_id steps/register/conf.yaml steps/evaluate/outputs/model_validation_status
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.register import RegisterStep; RegisterStep.from_step_config_path(step_config_path='{path:exe/steps/register/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/register/outputs}')"
...
ingest_scoring:
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.ingest import IngestScoringStep; IngestScoringStep.from_step_config_path(step_config_path='{path:exe/steps/ingest_scoring/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/ingest_scoring/outputs}')"
...
steps/predict/outputs/scored.parquet: steps/ingest_scoring/outputs/scoring-dataset.parquet steps/predict/conf.yaml
	...
	cd {path:prp/} && \
        python -c "from opengate.recipes.steps.predict import PredictStep; PredictStep.from_step_config_path(step_config_path='{path:exe/steps/predict/conf.yaml}', recipe_root='{path:prp/}').run(output_directory='{path:exe/steps/predict/outputs}')"
