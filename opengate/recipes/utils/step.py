...
from opengate.recipes.cards import pandas_renderer
...
def get_merged_eval_metrics(
    ...
    if len(missing_metrics) > 0:
        _logger.warning(
            "Input metric names %s not found in eval metrics: %s",
            missing_metrics,
            metric_names,
        )
        ordered_metric_names = [
...
def display_html(
    html_data: Optional[str] = None, html_file_path: Optional[str] = None
) -> None:
...
def truncate_pandas_data_profile(title: str, data_frame) -> str:
    ...
    max_rows = min(
        max(max_cells // max_cols, 1), len(data_frame), _MAX_PROFILE_ROW_SIZE
    )
