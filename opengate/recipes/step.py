...
from opengate.recipes.cards import (
    CARD_HTML_NAME,
    CARD_PICKLE_NAME,
    BaseCard,
    FailureCard,
)
from opengate.recipes.utils import get_recipe_name
...
class StepExecutionState:
    ...
    def from_dict(cls, state_dict) -> "StepExecutionState":
        ...
        return cls(
            ...
            last_updated_timestamp=state_dict[
                StepExecutionState._KEY_LAST_UPDATED_TIMESTAMP
            ],
...
class BaseStep(metaclass=abc.ABCMeta):
    ...
    def run(self, output_directory: str):
        ...
        try:
            self._update_status(
                status=StepStatus.RUNNING, output_directory=output_directory
            )
            self._validate_and_apply_step_config()
            self.step_card = self._run(output_directory=output_directory)
            self._update_status(
                status=StepStatus.SUCCEEDED, output_directory=output_directory
            )
        except Exception:
    ...
    @classmethod
    @abc.abstractmethod
    def from_recipe_config(
        cls, recipe_config: dict[str, Any], recipe_root: str
    ) -> "BaseStep":
        """
    ...
    @classmethod
    def from_step_config_path(
        cls, step_config_path: str, recipe_root: str
    ) -> "BaseStep":
        """
    ...
    def _serialize_card(self, start_timestamp: float, output_directory: str) -> None:
        ...
        if tab is not None:
            tab.add_markdown(
                "EXE_DURATION", f"**Run duration (s)**: {execution_duration:.3g}"
            )
            tab.add_markdown(
                "LAST_UPDATE_TIME",
    ...
    def _log_step_card(self, run_id: str, step_name: str) -> None:
        ...
        if os.path.exists(local_card_path):
            MlflowClient().log_artifact(
                run_id, local_card_path, artifact_path=step_name
            )
        else:
            _logger.warning(
