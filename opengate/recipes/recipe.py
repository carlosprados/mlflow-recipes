...
from opengate.recipes import dag_help_strings
from opengate.recipes.artifacts import Artifact
from opengate.recipes.step import BaseStep, StepClass, StepStatus
from opengate.recipes.utils import (
    get_recipe_config,
    get_recipe_name,
    get_recipe_root_path,
)
from opengate.recipes.utils.execution import (
...
class BaseRecipe:
    ...
    def __init__(self, recipe_root_path: str, profile: str) -> None:
        ...
        self._recipe = get_recipe_config(self._recipe_root_path, self._profile).get(
            "recipe"
        )
    ...
    def _get_recipe_dag_file(self) -> str:
        ...
        recipe_dag_template = j2_env.get_template(
            "resources/recipe_dag_template.html"
        ).render(
            {
                "recipe_yaml_help": {
        ...
        recipe_dag_file = os.path.join(
            get_or_create_base_execution_directory(self._recipe_root_path),
            "recipe_dag.html",
        )
...
class Recipe:
    ...
    def __new__(cls, profile: str):
        ...
        _logger.info(
            f"Creating MLflow Recipe '{recipe_name}' with profile: '{profile}'"
        )
